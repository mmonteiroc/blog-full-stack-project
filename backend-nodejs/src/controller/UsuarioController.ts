...
export class UsuarioController {
    ...
    private async editSecurity(req:Request, res:Response){
        ...
        * */
        if (usuario.authMode !== 'local') {
            res.status(BAD_REQUEST).statusMessage = 'oAuth users cannot change password ';
            return res.end();
        }

        /*
        * Miramos que la passwd recibida coincida con la del usuario
        * */
